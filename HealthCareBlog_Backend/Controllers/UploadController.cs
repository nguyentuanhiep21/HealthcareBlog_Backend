    ...
    public class UploadController : ControllerBase
    {
        ...
        public async Task<ActionResult> UploadBanner(IFormFile file)
        {
            try
            {
                ...
                // Upload to Supabase Storage
                var fileUrl = await _supabaseStorageService.UploadFileAsync(file, "Banners");
            ...
            {
                Console.WriteLine($"[UploadController] Error uploading banner: {ex.Message}");
                return StatusCode(500, new { message = "Đã xảy ra lỗi khi upload banner.", success = false });
            }
        }
    }
}
